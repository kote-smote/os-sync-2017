...
public class CalciumNitride {
    // Kontrolata e kaj tretiot atom na Calcium
    static Semaphore ca; // semafor za Calciumovite atomi
    static int caNum; // promenliva za kontrola koj po red Calciumov atom pristignal
    static Lock lock; // mutex za promenlivata caNum koja e vo kriticen region

    static Semaphore n; // semafor za Natriumovite atomi
    ...
    public static class Calcium extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ca.acquire();
            lock.lock();
            if (++caNum == 3) { // tretiot Ca atom sum, pa sigurno predhodnite dva Ca atomi se vekje prisutni
                caNum = 0;
                lock.unlock();
                nHere.acquire(2); // cekaj da pristignat dva Natriumovi atomi
                state.bond();
                canBond.release(4); // site atomi se tuka, signaliziraj deka site moze da zapocne so bond()
                canValidate.acquire(4); // cekaj site da zavrsat so bond() za da ja validiras novata molekula
                state.validate();
                done.release(4);
            } else { // prviot ili vtoriot atom sum
                lock.unlock();
                canBond.acquire();
                state.bond();
                canValidate.release();
                done.acquire(); // cekaj dodeka ne se zavrsi celosno procesot
            }
            ca.release();
        }
    }
    ...
    public static class Nitrogen extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            n.acquire();
            nHere.release(); // signaliziraj deka si pristignal
            canBond.acquire();
            state.bond();
            canValidate.release();
            done.acquire(); // cekaj dodeka ne se zavrsi celosno procesot
