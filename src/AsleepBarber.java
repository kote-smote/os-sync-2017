...
public class AsleepBarber {
    ...
    //static Semaphore done; // vo realnosta bi trebalo da go go ima sepaforov, no e e staven kako uslov vo zadacata
    static Semaphore customerArrived;
    static Semaphore canCut;
    static boolean barberIsSleeping;
    static int emptyChairs;
    ...
    static class Barber extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            // koga 5tiot klient ke signalizira, berberot treba da se razbudi
            if (barberIsSleeping) {
                wakeUp.acquire();
                state.barberWakeUp();
            ...
            state.barberCallCustomer(); // go povikuva klientot
            enter.release(); // mu dava znak na klientot da vleze
            canCut.acquire(); // ceka duri ne vleze klient za da go potstrizi
            state.cutHair();
            //done.release(); (ne e staveno kako uslov vo zadacata da mora da poceka da go potstrizi pa da plati)
            synchronized (lock) {
    ...
    static class Consumer extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            canCut.release(); // vlegol vo berbernicata i e spremen za potstrizuvanje
